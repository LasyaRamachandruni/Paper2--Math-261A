...
```{r}
#|label="table-descr", echo=FALSE
library(dplyr)
library(knitr)

descriptives <- analysis_wide %>%
  summarise(
    across(c(DIABETES, OBESITY, LPA, BPHIGH, CSMOKING, CHECKUP),
           list(
             N = ~n(),
             Mean = ~mean(.),
             SD = ~sd(.),
...
descriptives %>%
  kable(caption = "Table 1: Descriptive statistics for county-level variables")
...
Table @ref(tbl-descriptive-stats) displays summary statistics for the outcome (diabetes prevalence) and the five predictors across all counties analyzed. County-level diabetes prevalence ranges from X% to Y%, with an average of \~13.6% and standard deviation of Z%. Obesity, physical inactivity, and high blood pressure rates show similar variation, while smoking rates and routine checkup rates appear less variable.
