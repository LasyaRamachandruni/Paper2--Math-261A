...
After filtering for complete data on all six key measures, the final sample comprises `r n_final` counties. Excluding counties with missing values may introduce bias if missingness correlates with unobserved risk factors.
...
Table @ref(tbl-desc-stats) shows the mean, standard deviation, and range (min to max) for each of the six county-level variables used in the analysis.
